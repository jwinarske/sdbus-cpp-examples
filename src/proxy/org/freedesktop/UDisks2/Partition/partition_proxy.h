...
#ifndef __sdbuscpp__src_proxy_org_freedesktop_UDisks2_Partition_partition_proxy_h__proxy__H__
#define __sdbuscpp__src_proxy_org_freedesktop_UDisks2_Partition_partition_proxy_h__proxy__H__
...
class Partition_proxy {
 ...
 protected:
  Partition_proxy(sdbus::IProxy& proxy) : m_proxy(proxy) {}
 ...
 public:
  ...
  void SetName(const std::string& name,
               const std::map<std::string, sdbus::Variant>& options) {
    m_proxy.callMethod("SetName")
        .onInterface(INTERFACE_NAME)
        .withArguments(name, options);
  }
  ...
  void SetFlags(const uint64_t& flags,
                const std::map<std::string, sdbus::Variant>& options) {
    m_proxy.callMethod("SetFlags")
        .onInterface(INTERFACE_NAME)
        .withArguments(flags, options);
  }
 ...
 public:
  uint32_t Number() {
    return m_proxy.getProperty("Number")
        .onInterface(INTERFACE_NAME)
        .get<uint32_t>();
  }
  ...
  uint64_t Offset() {
    return m_proxy.getProperty("Offset")
        .onInterface(INTERFACE_NAME)
        .get<uint64_t>();
  }
  ...
  sdbus::ObjectPath Table() {
    return m_proxy.getProperty("Table")
        .onInterface(INTERFACE_NAME)
        .get<sdbus::ObjectPath>();
  }

  bool IsContainer() {
    return m_proxy.getProperty("IsContainer")
        .onInterface(INTERFACE_NAME)
        .get<bool>();
  }

  bool IsContained() {
    return m_proxy.getProperty("IsContained")
        .onInterface(INTERFACE_NAME)
        .get<bool>();
  }
