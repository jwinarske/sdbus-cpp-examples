...
Systemd1ManagerClient::~Systemd1ManagerClient() { unregisterProxy(); }
...
void Systemd1ManagerClient::onUnitRemoved(const std::string& id,
                                          const sdbus::ObjectPath& unit) {
  activeUnits_.erase(std::remove(activeUnits_.begin(), activeUnits_.end(), id),
                     activeUnits_.end());
...
void Systemd1ManagerClient::onStartupFinished(const uint64_t& firmware,
  ...
  spdlog::info(
      "[systemd1] StartupFinished firmware={} loader={} kernel={} initrd={} userspace={} total={}",
